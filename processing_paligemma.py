...
class PaliGemmaProcessor:
	...
	def __init(self, tokenizer, num_image_tokes: int, image_size: int):
		super().__init__()

		self.image_seq_length = num_image_tokes
		self.image_size = image_size

		# Tokenizer described here: https://github. com/google-research/big_vision/blob/main/big_vision/configs/proj/paligemma/README.md#tokenizer
		# Check hugging face - paligemma, paligemma-model card-3b 224
		tokens_to_add = {"additional_special_tokens": [self.IMAGE_TOKEN]}
		tokenizer.add_special_tokens(tokens_to_add)
		# Tokens for object detection (bounding boxes)
		EXTRA_TOKENS = [ f"<loc{i:04d}>" for i in range(1024)]
		# Tokens for object detection segmentation
	...
	def __call__(self, text: List[str], images: List[Image.Image], padding: str="longest", truncation: bool=True)) -> dict:
		...
		# input ids - position of each token in vocabulary
		The attention mask returned by a tokenizer is used to tell the model which 
		tokens should be attended to and which should be ignored during training or inference.
		"""
